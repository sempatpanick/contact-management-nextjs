...
export default function ContactSectionContactCard(
	props: ContactSectionContactCardProps
) {
	return (
		<>
			<div className="bg-gray-800 bg-opacity-80 rounded-xl shadow-custom border border-gray-700 overflow-hidden card-hover animate-fade-in">
				<div className="p-6">
					<Link
						href={`/dashboard/contacts/${props.contact?.id}`}
						...
						<div className="space-y-3 text-gray-300 ml-2">
							<p className="flex items-center">
								...
								<span>{props.contact?.first_name}</span>
							</p>
							<p className="flex items-center">
								<i className="fas fa-user-tag text-gray-500 w-6" />
								<span className="font-medium w-24">
								...
								<span>{props.contact?.last_name}</span>
							</p>
							<p className="flex items-center">
					...
					<div className="mt-4 flex justify-end space-x-3">
						<Link
							href={`/dashboard/contacts/${props.contact?.id}/edit`}
