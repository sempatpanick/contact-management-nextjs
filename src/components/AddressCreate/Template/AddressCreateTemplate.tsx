...
export default function AddressCreateTemplate() {
	...
	async function fetchContactDetail() {
		...
		if (!response.success) {
			alertError(response.message ?? "Failed to get contact");
			return;
		}
	...
	async function handleSubmit(e: FormEvent) {
		e.preventDefault();
		const response = await addressUseCase.createAddress(token ?? "", id, {
			street,
			city,
			province,
			country,
		...
		if (!response.success) {
			alertError(response.message ?? "Failed to create address");
			return;
		}
			...
			<div>
				<div className="flex items-center mb-6">
					...
					<h1 className="text-2xl font-bold text-white flex items-center">
						<i className="fas fa-plus-circle text-blue-400 mr-3" />{" "}
						Add New Address
					</h1>
				</div>
